...
const Navbar = () => {
  ...
  return (
    <nav style={{ backgroundColor: c.dark, padding: '0 32px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
      <Link to="/dashboard" style={{ color: c.white, fontWeight: 900, fontSize: 18, letterSpacing: 2, textDecoration: 'none' }}>ALETHEA</Link>
      <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
        ...
        <button onClick={handleLogout} style={{ background: 'none', border: `1px solid ${c.taupe}`, color: c.taupe, padding: '6px 14px', cursor: 'pointer', fontSize: 12, borderRadius: 4 }}>
          Logout
        </button>
      </div>
    </nav>
