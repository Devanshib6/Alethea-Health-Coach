      ...
      <nav style={{ backgroundColor: c.dark, padding: '0 32px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
          {[
            ...
            <Link key={i} to={item.path} style={{ color: i === 1 ? c.peach : c.taupe, textDecoration: 'none', fontSize: 13, letterSpacing: 1 }}>
              {item.label}
            </Link>
          ))}
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ backgroundColor: c.dark }}>
                <th style={{ padding: '14px 16px', color: c.white, textAlign: 'left', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontSize: 11 }}>User</th>
                <th style={{ padding: '14px 16px', color: c.peach, textAlign: 'left', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontSize: 11 }}>Email</th>
                <th style={{ padding: '14px 16px', color: c.peach, textAlign: 'left', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontSize: 11 }}>Role</th>
                <th style={{ padding: '14px 16px', color: c.peach, textAlign: 'left', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontSize: 11 }}>Status</th>
                <th style={{ padding: '14px 16px', color: c.peach, textAlign: 'left', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontSize: 11 }}>Joined</th>
                <th style={{ padding: '14px 16px', color: c.peach, textAlign: 'left', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, fontSize: 11 }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((u, i) => (
                  <tr key={u.id} style={{ borderTop: `1px solid ${c.peach}20`, backgroundColor: i % 2 === 0 ? c.white : `${c.peach}05` }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ 
                        backgroundColor: u.role === 'admin' ? c.dark : `${c.peach}20`, 
                        color: u.role === 'admin' ? c.white : c.dark, 
                        padding: '3px 10px', 
                        borderRadius: 20, 
                        fontSize: 11, 
                        fontWeight: 600, 
                    ...
                    <td style={{ padding: '14px 16px', color: c.taupe }}>{formatDate(u.created_at)}</td>
                    <td style={{ padding: '14px 16px' }}>
                      {u.id !== user?.id && (
                        <button 
      ...
      {showDeleteModal && (
        <div style={{ 
          ...
          <div style={{ backgroundColor: c.white, borderRadius: 12, padding: 32, maxWidth: 400, width: '90%' }}>
            <h3 style={{ color: c.dark, marginBottom: 12, fontWeight: 800 }}>Delete User?</h3>
